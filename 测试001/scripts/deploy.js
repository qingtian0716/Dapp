...
async function main() {
  try {
    ...
    console.log("正在部署合约...");
    // 修复：传入管理员地址作为构造函数参数
    const yourCollectible = await YourCollectible.deploy(deployer.address);
    ...
    const addressPath = path.join(__dirname, '../address/YourCollectible_address.json');
    const addressData = {
    ...
    const abiPath = path.join(__dirname, '../abi/YourCollectible_ABI.json');
